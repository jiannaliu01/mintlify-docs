...
Cardinal's schema extraction lets you define the structure you want and get structured data back.

- Fields missing in the document return null
- Fields not in your schema are ignored
- Arrays are supported (see long-doc caveats below)

**Endpoint:** `POST /extract` (supports PDF and images via upload or fileUrl)  
...
### Fast Schema — `/extract` with `fast: true`

Use Fast when you need a quick, low-latency, lower-cost pass.

- Best for short docs or targeted checks
